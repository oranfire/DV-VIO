...
void FeatureTracker::readImage(const cv::Mat &_img, double _cur_time, bool discard_feature)
{
    ...
    if (cur_pts.size() > 0)
    {
        ...
        if (OPTICAL_FLOW_BACK)
        {
            ...
            calcOpticalFlowPyrLK(forw_img, cur_img, forw_pts, reverse_pts, reverse_status, err, cv::Size(21, 21), 3, 
                cv::TermCriteria(cv::TermCriteria::COUNT+cv::TermCriteria::EPS, 30, 0.01), cv::OPTFLOW_USE_INITIAL_FLOW);
            for (int i = 0; i < status.size(); i++)
            {
                if (status[i] && reverse_status[i] && distance(reverse_pts[i], cur_pts[i]) <= 0.5)
                    status[i] = 1;
                else
                    status[i] = 0;
            }
...
{
    if (forw_pts.size() >= 8 && F_THRESHOLD > 0)
    {
