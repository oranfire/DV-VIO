...
void pubOdometry(const Estimator &estimator, const std_msgs::Header &header)
{
    if (estimator.solver_flag == Estimator::SolverFlag::NON_LINEAR)
    {
        ...
        {
            Quaterniond dio_q(estimator.dio_manager->trajs.back().R);
            ofstream foutD(DIO_RESULT_PATH, ios::app);
            foutD.setf(ios::fixed, ios::floatfield);
            foutD.precision(10);
            foutD << estimator.dio_manager->trajs.back().time_stamp << " ";
            foutD.precision(5);
            foutD << estimator.dio_manager->trajs.back().P.x() << " "
                << estimator.dio_manager->trajs.back().P.y() << " "
                << estimator.dio_manager->trajs.back().P.z() << " "
                << dio_q.x() << " "
                << dio_q.y() << " "
                << dio_q.z() << " " 
                << dio_q.w() << endl;
